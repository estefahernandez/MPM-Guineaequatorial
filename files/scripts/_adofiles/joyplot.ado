...
program joyplot
	...
	qui:frame `frame': {
		...
		** Línea de referencia vertical (xline), dibujada como un plot más
		** para que quede POR ENCIMA de las áreas del joyplot: la sub-opción
		** xline() nativa de twoway se dibuja detrás de los datos graficados,
		** por lo que las áreas semi-opacas del joyplot la tapaban.
		if `"`xline'"' != "" {
			local xcomma = strpos(`"`xline'"', ",")
			if `xcomma' > 0 {
				local xnums = substr(`"`xline'"', 1, `xcomma'-1)
				local xopts = substr(`"`xline'"', `xcomma'+1, .)
			}
			else {
				local xnums `"`xline'"'
				local xopts
			...
			local ytop = `dadj' + 1
			foreach xv of numlist `xnums' {
				local joy `joy' (scatteri 0 `xv' `ytop' `xv', recast(line) `xopts')
			}
		}
		***************************************************************************************************************
